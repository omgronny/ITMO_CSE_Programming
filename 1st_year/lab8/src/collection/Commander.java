...
import interfaces.MainStage;
import interfaces.RegisterStage;
import interfaces.RemoveStage;
...
public class Commander implements Invoker {
    ...
    public static void setReadBuffer(ByteBuffer readBuffer) {
        Commander.readBuffer = readBuffer;
    }
    ...
    public static void setWriteBuffer(ByteBuffer writeBuffer) {
        Commander.writeBuffer = writeBuffer;
    }

    private static String login;
    private static String password;

    private static boolean isShower = false;

    public static void setIsShower(boolean isShower) {
        Commander.isShower = isShower;
    }

    public static boolean getIsShower() {
        return isShower;
    }

    public void send(byte[] serializedObject, SocketChannel writeSocket) {

        writeBuffer.clear();

        try {
            writeSocket.configureBlocking(false);
        } catch (IOException e) {
            System.out.println("Ошибка при переключении на неблокирующий режим");
        }

        writeBuffer.put(serializedObject);
        writeBuffer.flip();

        try {
            writeSocket.write(writeBuffer);
        } catch (IOException e) {
    ...
    public String signIn(Selector selector, RegisterStage registerStage, SocketChannel writeSocket, SocketChannel readSocket) throws IOException {
        ...
        while (true) {
            ...
            Register register = registerStage.getRegister();

            try {
                if (register.getLogin() == null || register.getPassword() == null) {
                    continue;
                }
            ...
            Register register1 = new Register(login, password);

            System.out.println(login + " " + password);

            byte[] testBytes = register1.toBytes();
            ...
            while (true) {
                ...
                try {
                    ...
                    nowAnswer = Command.fromBytesToString(readBuffer.array());
                    ...
                    if (!nowAnswer.equals("Неверный пароль")) {
                        Runnable runnable = () -> {
                            while (true) {
                                try {
                                    Thread.sleep(5000);
    ...
    public void consoleInput(File scriptFile, Scanner in, SocketChannel s)
            throws IOException, CancelledKeyException, ValueException {
...
//            }
//
//        } while (nowAnswer.equals("Неверный пароль"));
